...
# We need members intents in order to query discord members
intents.members = True
# This is where we define the bot on discord
discord_bot = commands.Bot(command_prefix='`@`', description=description, intents=intents)
...
async def discord_mute(sopel_bot, user_name, reason):
    ...
    member_object = await lookup_discord_member(sopel_bot, user_name)
    if member_object:
        await member_object.add_roles(muted_role, reason=f"Muted via IRC for: {reason}")
        logger.info(f"Muted {member_object.name}#{member_object.discriminator} ({member_object.nick}) for {reason}")
        sopel_bot.reply(f"Muted {member_object.name}#{member_object.discriminator} ({member_object.nick}) for {reason}")
...
async def discord_unmute(sopel_bot, user_name):
    member_object = await lookup_discord_member(user_name)
    if member_object:
        await member_object.remove_roles(muted_role, reason="Unmuted via IRC")
        logger.info(f"Unmuted {member_object.name}#{member_object.discriminator} ({member_object.nick})")
        sopel_bot.reply(f"Unmuted {member_object.name}#{member_object.discriminator} ({member_object.nick})")
...
async def discord_kick(sopel_bot, user_name, reason):
    member_object = await lookup_discord_member(sopel_bot, user_name)
    if member_object:
        logger.debug(f"Attempting to kick {member_object}")
        try:
            await guild.kick(member_object, reason=f"Kicked via IRC for: {reason}")
        except Exception as e:
            logger.critical(e)
            sopel_bot.reply(e)
        logger.info(f"Kicked {member_object.name}#{member_object.discriminator} ({member_object.nick}) for {reason}")
        sopel_bot.reply(f"Kicked {member_object.name}#{member_object.discriminator} ({member_object.nick}) for {reason}")
...
async def discord_ban(sopel_bot, user_name, reason):
    member_object = await lookup_discord_member(sopel_bot, user_name)
    if member_object:
        logger.debug(f"Attempting to ban {member_object}")
        try:
            await guild.ban(member_object, reason=f"Kicked via IRC for: {reason}", delete_message_days=7)
        except Exception as e:
            logger.critical(e)
            sopel_bot.reply(e)
        logger.info(f"Banned {member_object.name}#{member_object.discriminator} ({member_object.nick}) for {reason}")
        sopel_bot.reply(f"Banned {member_object.name}#{member_object.discriminator} ({member_object.nick}) for {reason}")
...
async def discord_rename(sopel_bot, user_name, new_name):
    member_object = await lookup_discord_member(sopel_bot, user_name)
    if member_object:
        logger.debug(f"Attempting to change name on {member_object}")
        try:
            await member_object.edit(nick=new_name)
        except Exception as e:
            logger.critical(e)
            sopel_bot.reply(e)
        logger.info(f"Renamed {user_name} to {new_name}")
        sopel_bot.reply(f"Renamed {user_name} to {new_name}")
...
def setup(bot):
    ...
    logger.info(f"GuildID: {guild_id} | MutedID: {muted_role_id} | BotspamID: {botspam_channel_id} | SandboxID: {sandbox_channel_id}")
...
async def on_ready():
    ...
    logger.info(f"Connected to {guild.name} discord and outputting to the {sandbox_channel.name} room")
    await sandbox_channel.send(f"Connected to {guild.name} discord and outputting to the {sandbox_channel.name} room")
    global muted_role
...
async def lookup_discord_member(sopel_bot, user_name):
    ...
    logger.debug(f"Processing {user_name}")
    try:
        # Try to turn the username into an integeger, in case the UserID was used
        user_name = int(user_name)
        logger.debug(f"Looking for the user ID {user_name}")
        matching_members = await guild.query_members(user_ids=[user_name])
    except ValueError:
        logger.debug(f"Looking for the user name {user_name}")
        matching_members = await guild.query_members(query=user_name)
    logger.debug(matching_members)
    if not matching_members:
        logger.debug(f"Could not find {user_name} on this server!")
        sopel_bot.reply(f"Could not find {user_name} on this server!")

    if len(matching_members) > 1:
        logger.debug(f"Multiple results for {user_name}, pick one:")
        sopel_bot.reply(f"Multiple results for {user_name}, pick one:")
        for member in matching_members:
            logger.debug(f"Nickname: {member.nick} | DiscordID: {member.name}#{member.discriminator} | UserID: {member.id}")
            sopel_bot.reply(f"Nickname: {member.nick} | DiscordID: {member.name}#{member.discriminator} | UserID: {member.id}")
    if len(matching_members) == 1:
        logger.debug(f"Found a single match for {user_name}")
